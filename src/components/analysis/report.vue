    ...
    <div class="filters">
        <div class="filters-content">
            <div class="filters-item first">
                <h3>上传人群：</h3>
                <span v-if="crowd&&crowd.name" v-bind:data-id="crowd.id">{{crowd.name}}
                    <i class="delete"></i>
                </span>
            ...
            <div class="last-two">
                <div class="filters-item second">
                    <h3>标签筛选：</h3>
                    <span v-for="item in filters" v-bind:basketid="item.index" v-bind:title="item.title">{{item.text}}
                        <i class="delete"></i>
                    </span>
                    <span v-for="item in fixedpros" v-bind:tagid="item.code">{{item.text}}
                        <i class="delete"></i>
                    </span>
                </div>
                <div class="filters-item third">
                    <h3>建模筛选：</h3>
                    <span v-if="model" v-bind:model="model.id">{{model.name}}
                        <i class="delete"></i>
                    </span>
